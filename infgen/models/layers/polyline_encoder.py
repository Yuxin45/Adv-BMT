...
class PointNetPolylineEncoder(nn.Module):
    def __init__(self, in_channels, hidden_dim, num_layers=3, num_pre_layers=1, out_channels=None, is_v7=False):
        super().__init__()
        self.pre_mlps = common_layers.build_mlps(
            c_in=in_channels,
            mlp_channels=[hidden_dim] * num_pre_layers,
            ret_before_act=False,  #is_v7=is_v7
        )
    ...
    def forward(self, polylines, polylines_mask):
        ...
        polylines_feature = polylines_feature_valid.new_zeros(
